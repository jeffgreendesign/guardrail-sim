...
- `analyze_simulation` - Run a simulation and surface policy health insights

**UCP checkout**

- `create_checkout` - Create a checkout session
- `get_checkout` - Retrieve a session by id
- `update_checkout` - Update a session and re-evaluate discounts
- `complete_checkout` - Complete a session, producing an order reference
- `cancel_checkout` - Cancel a session
