...
<Callout title="Not conformance-tested">
  This table describes what the types and the MCP server are written against. The project does not
  run the UCP conformance suite in CI, so nothing here is a certification claim.
</Callout>
